...
if ( VW_CONST_ENABLE_SHORTCODES ) {
	...
	if ( is_admin() ) {
		require_once get_template_directory().'/components/shortcode-editor/shortcode-editor.php';
	}
}
...
if ( ! function_exists( 'vw_shortcode_posts' ) ) {
	function vw_shortcode_posts( $atts, $content = null ) {
		...
		// Option: sort
		if ( ! empty( $sort ) ) {
			$query_args['order'] = strtoupper( $sort );
		}
		...
		<div id="vw_post_shortcode_id_<?php echo esc_attr( ++$vw_posts_shortcode_id ); ?>" class="vw-post-shortcode">
			<?php if ( ! empty ( $title ) ) : ?>
			<h2 class="vw-post-shortcode-title vw-right-topic-line"><?php echo $title; ?></h2>
			<?php endif; ?>
...
if ( ! function_exists( 'vw_shortcode_pricing_table' ) ) {
	function vw_shortcode_pricing_table( $atts, $content = null ) {
		...
		$html = str_replace( '##VW_COLUMN_SIZE##', $col_class, $html );
...
if ( ! function_exists( 'vw_shortcode_pricing_item' ) ) {
	function vw_shortcode_pricing_item( $atts, $content = null ) {
		...
		ob_start();
		?>
		<div class="vw-pricing-item <?php if ( $featured ) echo esc_attr( 'vw-pricing-featured' ); ?> vw-column-shortcode ##VW_COLUMN_SIZE##">
