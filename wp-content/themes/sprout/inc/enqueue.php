...
if ( ! function_exists( 'vw_register_scripts' ) ) {
	function vw_register_scripts() {
		...
		if ( is_admin() ) {
			...
			// Font icons
			wp_enqueue_style( 'vwcss-icon-iconic', get_template_directory_uri().'/components/font-icons/iconic/css/iconic.css', array(), VW_THEME_VERSION );
			...
			if ( vw_get_theme_option( 'icon_entypo' ) )		wp_enqueue_style( 'vwcss-icon-entypo', get_template_directory_uri().'/components/font-icons/entypo/css/entypo.css', array(), VW_THEME_VERSION );
			...
			// wp_enqueue_style( 'vwcss-animate', get_template_directory_uri().'/css/animate.css', array(), VW_THEME_VERSION );
			wp_enqueue_style( 'vwcss-bootstrap', get_template_directory_uri().'/css/bootstrap.css', array(), VW_THEME_VERSION );		
			wp_enqueue_script( 'vwjs-backstretch', get_template_directory_uri().'/js/jquery-backstretch/jquery.backstretch.min.js', array( 'jquery' ), VW_THEME_VERSION, VW_CONST_ENQUEUE_SCRIPTS_ON_FOOTER );
			wp_enqueue_script( 'vwjs-bxslider', get_template_directory_uri().'/js/jquery-bxslider/jquery.bxslider.min.js', array( 'jquery' ), VW_THEME_VERSION, VW_CONST_ENQUEUE_SCRIPTS_ON_FOOTER );
			...
			wp_enqueue_script( 'vwjs-imagesloaded', get_template_directory_uri().'/js/imagesloaded.pkgd.min.js', array( 'jquery' ), VW_THEME_VERSION, VW_CONST_ENQUEUE_SCRIPTS_ON_FOOTER );
			...
			wp_enqueue_script( 'vwjs-magnific-popup', get_template_directory_uri().'/js/jquery.magnific-popup.min.js', array( 'jquery' ), VW_THEME_VERSION, VW_CONST_ENQUEUE_SCRIPTS_ON_FOOTER );
			...
			wp_localize_script( 'vwjs-main', 'vw_main_js', apply_filters( 'vw_filter_localize_main_js', array(
				...
				'bxslider_auto_start' => vw_get_theme_option( 'bxslider_auto_start' ),
				'bxslider_pause_on_hover' => vw_get_theme_option( 'bxslider_pause_on_hover' ),
				'bxslider_slide_duration' => vw_get_theme_option( 'bxslider_slide_duration' ),
				'bxslider_transition_speed' => vw_get_theme_option( 'bxslider_transition_speed' ),
				
				'bg_ads_enable' => vw_get_theme_option( 'bg_ads_enable' ),
				'bg_ads_left_url' => vw_get_theme_option( 'bg_ads_left_url' ),
				'bg_ads_right_url' => vw_get_theme_option( 'bg_ads_right_url' ),
			) ) );
			...
			if ( vw_is_rtl() ) {
				...
				wp_enqueue_style( 'vwcss-theme-rtl-2', get_template_directory_uri().'/rtl.css', array(), VW_THEME_VERSION );
