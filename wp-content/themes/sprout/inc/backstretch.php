...
if ( ! function_exists( 'vw_enqueue_scripts_backstretch' ) ) {
	function vw_enqueue_scripts_backstretch() {
		...
		if ( is_page() && ! is_page_template( 'page_big_featured_image.php' )
		 || ( vw_is_shop() && get_page_template_slug( vw_get_shop_page_id() ) != 'page_big_featured_image.php' ) ) {
			return;
		}
				...
				foreach ( $attachments as $attachment_ID ) {
					...
					if ( $full_image_url ) {
						...
						if ( ! empty( $image_caption ) ) {
							$image_captions[] = sprintf( '<div class="vw-featured-image-caption hidden"><i class="vw-icon icon-iconic-camera"></i> %s</div>', $image_caption );
						}
					}
				}
			}
			
			if ( ( is_single() && has_post_thumbnail() && empty( $image_urls ) ) || is_page() || vw_is_shop() ) {
				...
				if ( ! empty( $attachment ) && ! empty( $attachment->post_excerpt ) ) {
					$image_captions[] = sprintf( '<div class="vw-featured-image-caption hidden"><i class="vw-icon icon-iconic-camera"></i> %s</div>', $attachment->post_excerpt );
				}
			}
